	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version().then((info) => {
				...
				util.info('retrying connection to Chrome...');
	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Network.requestWillBeSent((params) => {
				...
				if (tab.prerender.logRequests || this.options.logRequests) util.info('+', tab.prerender.numRequestsInFlight, params.request.url);
			...
			Network.responseReceived((params) => {
				...
				if(tab.prerender.requests[params.requestId]) {
					...
					if (tab.prerender.logRequests || this.options.logRequests) util.info('-', tab.prerender.numRequestsInFlight, tab.prerender.requests[params.requestId]);
			...
			Console.messageAdded((params) => {
				if (tab.prerender.logRequests || this.options.logRequests) {
					...
					util.info('level:', message.level, 'text:', message.text, 'url:', message.url, 'line:', message.line);
				}
			});

			Log.entryAdded((params) => {
				if (tab.prerender.logRequests || this.options.logRequests) util.info(params.entry);
			});
		...
		.then(() => {
			...
			var checkIfDone = () => {
				...
				this.checkIfPageIsDoneLoading(tab).then((doneLoading) => {
					...
					finished = true;
					util.error('Chrome connection closed during request');
			...
			setTimeout(() => {
				if (!finished) {
					finished = true;
					util.error('page timed out', tab.prerender.url);
					resolve();
				}
			}, pageLoadTimeout);
			...
			Page.navigate({
				...
				util.error('invalid URL sent to Chrome:', tab.prerender.url);
				tab.prerender.statusCode = 504;
				finished = true;
				reject();
			});
		}).catch((err) => {
			util.error('unable to load URL', err);
			tab.prerender.statusCode = 504;
			finished = true;
			reject();
		});
...
chrome.checkIfPageIsDoneLoading = function(tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			util.error('unable to evaluate javascript on the page');
			tab.prerender.statusCode = 504;
			reject();
		});
	});
		...
		tab.Runtime.evaluate({
			...
			setTimeout( () => {
				...
				tab.Runtime.evaluate({
					...
					util.error('unable to evaluate javascript on the page', err);
					tab.prerender.statusCode = 504;
					reject();
				});
...
chrome.parseHtmlFromPage = function(tab) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.error('parse html timed out', tab.prerender.url);
			tab.prerender.statusCode = 504;
			reject();
		}, 5000);
...
chrome.captureScreenshot = function(tab, format, fullpage) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.error('capture screenshot timed out for', tab.prerender.url);
			tab.prerender.statusCode = 504;
			reject();
		}, 10000);
		...
		tab.Page.getLayoutMetrics().then((viewports) => {
			...
			tab.Page.captureScreenshot({
				...
				util.error('unable to capture screenshot:', err);
				tab.prerender.statusCode = 504;
				clearTimeout(parseTimeout);
				reject();
			});
...
chrome.printToPDF = function(tab, options) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.error('print pdf timed out for', tab.prerender.url);
			tab.prerender.statusCode = 504;
			reject();
		}, 5000);
		...
		tab.Page.printToPDF(options).then((response) => {
			...
			util.error('unable to capture pdf:', err);
			tab.prerender.statusCode = 504;
			clearTimeout(parseTimeout);
			reject();
		});
