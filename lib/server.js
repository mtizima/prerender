...
server.startPrerender = function() {
	return new Promise((resolve, reject) => {
		this.spawnBrowser().then(() => {
			...
			util.info(`Started ${this.browser.name}: ${this.browser.version}`)
			resolve();
		}).catch((err) => {
			util.error(err);
...
server.spawnBrowser = function() {
	...
	util.info(`Starting ${this.browser.name}`);
...
server.killBrowser = function() {
	util.info(`Stopping ${this.browser.name}`);
...
server.restartBrowser = function() {
	this.isBrowserConnected = false;
	util.info(`Restarting ${this.browser.name}`);
	this.browser.kill();
};
...
server.listenForBrowserClose = function() {
	...
	this.browser.onClose(() => {
		...
		if(this.isBrowserClosing) {
			util.info(`Stopped ${this.browser.name}`);
			return;
		}

		util.info(`${this.browser.name} connection closed... restarting ${this.browser.name}`);

		if (new Date().getTime() - start < 1000) {
			util.error(`${this.browser.name} died immediately after restart... stopping Prerender`);
			return process.exit();
		}
	...
	.then(() => {
		...
		setTimeout(() => {
			if (!req.prerender.responseSent) {
				util.error('response not sent for', req.prerender.url);
...
server._send = function(req, res) {
	...
	if (req.prerender.renderType == 'html') {
		Object.keys(req.prerender.headers || {}).forEach(function(header) {
			try {
				...
				util.info('warning: unable to set header:', header);
			}
		});
	}
	...
	util.info('got', req.prerender.statusCode, 'in', ms + 'ms', 'for', req.prerender.url);
};
